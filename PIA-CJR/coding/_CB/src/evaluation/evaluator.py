...
class Evaluator:
    ...
    def __init__(self, model, dataloader, device, config, class_names, run_id):
        ...
        self.output_dir = Path(config["EVALUATION_DIR"]) / run_id
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(self.dataloader, desc="Evaluation"):
                ...
                preds = (positive_probs >= self.config["DEFECT_THRESHOLD"]).long()

                confs = torch.where(preds == 1, positive_probs, 1 - positive_probs)
        ...
        self._save_predictions_csv(image_paths, y_true, y_pred, y_prob, confidences)
        ...
        save_confusion_matrix(
            ...
            normalize=self.config["NORMALIZE_CONFUSION_MATRIX"],
        )
        ...
        self.save_roc_curve(y_true, y_prob)

        # ====================================================
        # PRECISION-RECALL CURVE
        # ====================================================

        self.save_precision_recall_curve(y_true, y_prob)
    ...
    def _save_predictions_csv(self, image_paths, y_true, y_pred, y_prob, confidences):
    ...
    def save_roc_curve(self, y_true, y_prob):
    ...
    def save_precision_recall_curve(self, y_true, y_prob):
        ...
        save_path = self.figures_dir / "pr_curve.png"
