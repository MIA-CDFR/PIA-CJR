...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(
            ...
            train_probs.extend(
                positive_probs.detach().cpu().numpy()
            )
        ...
        history["val_roc_auc"].append(
            val_metrics.get("roc_auc", 0)
        )

        history["val_f1_defect"].append(
            val_metrics["f1_defect"]
        )

        history["val_f1_non_defect"].append(
            val_metrics["f1_non_defect"]
        )
        ...
        logger.info(
            f"Train ROC AUC: "
            f"{train_metrics.get('roc_auc', 0):.4f}"
        )
        ...
        logger.info(
            ...
            f"{val_metrics['precision_weighted']:.4f}"
        )

        logger.info(
            f"Validation Recall: "
            f"{val_metrics['recall_weighted']:.4f}"
        )
        ...
        logger.info(
            f"Defect F1: "
            f"{val_metrics['f1_defect']:.4f}"
        )

        logger.info(
            f"Non-Defect F1: "
            f"{val_metrics['f1_non_defect']:.4f}"
        )
