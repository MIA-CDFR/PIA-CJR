...
def build_blue_mask(img: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    _, thresh = cv2.threshold(blue_raw, PARAMS["blue_threshold"], 255, cv2.THRESH_BINARY)
...
def rotate_and_crop(src: np.ndarray, cx: float, cy: float,
...
def extract_panels(img_path: str, output_dir: str, debug: bool = False) -> list[dict]:
    ...
    if debug:
        _save_debug_overview(img, row_contours, output_dir / "debug" / f"{base_name}_overview.jpg")
...
def _save_debug_overview(img, row_contours, out_path):
    ...
    for i, cnt in enumerate(row_contours):
        ...
        cv2.putText(debug_img, f"Fila {i}", (cx-60, cy),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, colors[i % len(colors)], 5)
...
def main():
    ...
    parser.add_argument("--output", "-o", default="./paineis_extraidos",
