...
class ConvAutoEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(
                64, 128, 3, stride=2, padding=1, output_padding=1
            ),  # 16×16
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(
...
def train(
    ...
    plt.ylabel("MSE Loss", color="gray")
...
# ──────────────────────────────────────────────
def predict(
    model: ConvAutoEncoder, image_paths: list[str], device: torch.device
) -> list[dict]:
    """
    ...
    with torch.no_grad():
        for path in image_paths:
            ...
            t = (
                torch.tensor(img_128, dtype=torch.float32).unsqueeze(0).unsqueeze(0)
                / 255.0
            ).to(device)
...
def _score_color(score: float) -> str:
    if score > PARAMS["score_red"]:
        return "#ff4444"
    if score > PARAMS["score_yellow"]:
        return "#ffaa00"
    return "#44ff88"
...
def save_detailed_report(results: list[dict], output_dir: Path):
    ...
    for i, r in enumerate(results[:n]):
        ...
        ax_over.set_title(
            f"Score: {r['score']:.0f}/100", color=color, fontsize=10, fontweight="bold"
        )
        ax_over.axis("off")
...
def save_overview(results: list[dict], output_dir: Path):
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(cols * 2.6, rows * 3.0), facecolor="#0f0f0f"
    )
    ...
    for i, r in enumerate(results):
        ...
        color = _score_color(r["score"])
        ax.set_title(
            f"#{i+1}  {r['name']}\n{r['score']:.0f}/100", color=color, fontsize=6.5
        )
        ax.axis("off")
        ax.set_facecolor("#0f0f0f")
    ...
    data = [
        {
            ...
            "alert": (
                "RED"
                if r["score"] > PARAMS["score_red"]
                else "YELLOW" if r["score"] > PARAMS["score_yellow"] else "GREEN"
            ),
        }
        for i, r in enumerate(results)
    ]
...
def main():
    ...
    parser.add_argument(
        "--panels", "-p", required=True, help="Directório com painéis extraídos (.png)"
    )
    parser.add_argument(
        "--output", "-o", default="./anomaly_results", help="Directório de output"
    )
    parser.add_argument(
        "--epochs",
        "-e",
    ...
    parser.add_argument(
        "--model", "-m", default=None, help="Caminho para modelo .pth já treinado"
    )
    parser.add_argument(
        "--no-train", action="store_true", help="Não treina — usa --model diretamente"
    )
    ...
    image_paths = sorted(
        [str(p) for p in panels_dir.iterdir() if p.suffix.lower() in exts]
    )
    if not image_paths:
