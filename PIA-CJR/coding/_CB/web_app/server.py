...
def predict():
    ...
    try:
        ...
        # Validar extensão de ficheiro
        valid_extensions = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.tif', '.webp'}
        file_ext = Path(file.filename).suffix.lower()
        if file_ext not in valid_extensions:
            return jsonify({"error": f"Invalid image format: {file_ext}. Allowed: {', '.join(valid_extensions)}"}), 400
        ...
        try:
            ...
            return jsonify({"error": f"Failed to open image: {str(e)}. The file may be corrupted or not a valid image."}), 400
